...
class MenuController{
    ...
    public function inicio(){
        try{
            ...
            $grupos__m = $this->nav->listar_grupos_m();
    ...
    public function agrupaciones(){
        try{
            $this->nav = new Navbar();
            $grupos = $this->nav->listar_grupos($this->encriptar->desencriptar($_SESSION['ru'],_FULL_KEY_));
            $grupos_v = $this->nav->listar_grupos_v();
            $roles = $this->rol->listar_roles();

            require _VIEW_PATH_ . 'header.php';
            require _VIEW_PATH_ . 'navbar.php';
            require _VIEW_PATH_ . 'menu/agrupaciones.php';
            require _VIEW_PATH_ . 'footer.php';
        }
        catch (Throwable $e){
            $this->log->insertar($e->getMessage(), get_class($this).'|'.__FUNCTION__);
    ...
    public function guardar_menu(){
        ...
        try{
            ...
            $ok_data = $this->validar->validar_parametro('id_grupo', 'POST',false,$ok_data,11,'numero',0);
            //Validacion de datos
            if($ok_data){
    ...
    public function guardar_grupo(){
        ...
        try{
            ...
            if($ok_data){
                $id_grupo = $_POST['id_grupo'];
                if($id_grupo == null){
                    $validar_nombre = $this->menu->validar_grupo($_POST['grupo_nombre']);
                    if($validar_nombre){
                        $result = 3;
                    }else{
                        $result = $this->builder->save("grupos", array(
                            "grupo_nombre" => $_POST['grupo_nombre'],
                            "grupo_icono" => $_POST['grupo_icono'],
                            "grupo_orden" => $_POST['grupo_orden'],
                            "grupo_mostrar" => $_POST['grupo_mostrar'],
                            "grupo_estado" => $_POST['grupo_estado'],
                        ));
                    }
                }else{
                    $validar_nombre_edicion = $this->menu->validar_nombre_edicion($_POST['grupo_nombre'],$id_grupo);
                    if($validar_nombre_edicion){
                        $result = 3;
                    }else{
                        $result = $this->builder->update("grupos",array(
                            "grupo_nombre" => $_POST['grupo_nombre'],
                            "grupo_icono" => $_POST['grupo_icono'],
                            "grupo_orden" => $_POST['grupo_orden'],
                            "grupo_mostrar" => $_POST['grupo_mostrar'],
    ...
    public function agregar_permisos_grupo(){
        $result = 2;
        $message = 'OK';
        try{
            $ok_data = true;
            if($ok_data){
                $id_grupo = $_POST['guardarid'];
                $id_rol = $_POST['guardarid_ii'];
                $validar = $this->menu->doble_permiso($id_grupo,$id_rol);
                if($validar){
                    $result = 2;
                }else{
                    $result = $this->builder->save("roles_grupos", array(
                        "id_rol" => $id_rol,
                        "id_grupo" => $id_grupo,
                    ));
                }
    ...
    public function eliminar_permisos_grupo(){
        $result = 2;
        $message = 'OK';
        try{
            $ok_data = true;
            if($ok_data){
                $id_grupo = $_POST['guardarid'];
                $id_rol = $_POST['guardarid_ii'];
                $result = $this->builder->delete("roles_grupos", array(
                    "id_rol" => $id_rol,
                    "id_grupo" => $id_grupo,
                ));
            } else {
