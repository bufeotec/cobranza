...
try{
    $logo = EscposImage::load( "media/logo_empresa.png", false);
...
$printer->text("$empresa->empresa_domiciliofiscal" . "\n");
$printer->text("LORETO - MAYNAS - BELEN" . "\n\n");
...
//$printer->text("ACTIVIDADES DE COSTRUCCIÓN Y SERVICIOS" . "\n\n");
//$printer->text("menor y mayor precio" . "\n\n");
$printer->setFont(Printer::FONT_B);
$printer->setTextSize(2,2);
...
if($cliente->cliente_numero == "11111111"){
    $numerocliente = "SIN DOCUMENTO";
}else{
    $numerocliente = "$cliente->cliente_numero";
}
$printer->text("Nro. Doc    : $numerocliente" . "\n");
...
$total = 0;
$printer->text("DESCRIPCIÓN                P/U            TOTAL ");
foreach ($detalle_venta as $dp) {
    $total += $dp->venta_detalle_cantidad * $dp->venta_detalle_valor_unitario;

    /*Alinear a la izquierda para la cantidad y el nombre*/
    $printer->setJustification(Printer::JUSTIFY_LEFT);
    $printer->text("[$dp->venta_detalle_cantidad]".$dp->venta_detalle_nombre_producto . "\n");

    /*Y a la derecha para el importe*/
    $printer->setJustification(Printer::JUSTIFY_RIGHT);
    $printer->text($dp->venta_detalle_valor_unitario.'        S/ ' . $dp->venta_detalle_valor_total . "\n");
}
...
$printer->setTextSize(1,1);
$printer->setJustification(Printer::JUSTIFY_CENTER);
$montoLetras = $this->numeroletras->toInvoice($venta->venta_total, 2, 'SOLES');
$printer->text($montoLetras ."\n");
$printer->text("------------------------------------------------" . "\n");
...
$printer->setFont(Printer::FONT_B);
$printer->setTextSize(1,1);
$printer->text("Bufeo Tec Company" . "\n");
$printer->text("Digitalízate" . "\n");
...
*/
$printer->close();
//header("Location: "._SERVER_."Ventas/realizar_venta");
?>
