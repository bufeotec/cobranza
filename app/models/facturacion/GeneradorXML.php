 ...
 * Time: 09:32 a. m.
 */
require 'app/models/facturacion/cantidad_en_letras.php';
require 'app/models/facturacion/Numberletras.php';
class GeneradorXML
{
    ...
    function CrearXMLFactura($nombrexml, $emisor, $cliente, $comprobante, $detalle, $detallecuotas = [])
    {
        ...
        //SI EL PRIMER CARACTER ES ESPACION REEMPLAZAMOS A NADA
        //$total_letras = str_replace(' MIL', 'MIL', $total_letras);
        $total_letras = $this->numeroletras->toInvoice($comprobante->venta_total, 2, $comprobante->moneda);
      ...
      <Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2" xmlns:ds="http://www.w3.org/2000/09/xmldsig#" xmlns:ext="urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2">
        ...
        //tipo de pago
        if($comprobante->venta_forma_pago == "CONTADO"){
            ...
            /*$dia_cuota = date('Y-m-d', strtotime("{$comprobante->venta_fecha} + 15 days") );
            $xml.='<cac:PaymentTerms>
                        <cbc:ID>FormaPago</cbc:ID>
                        <cbc:PaymentMeansID>Credito</cbc:PaymentMeansID>
                        <cbc:Amount currencyID="PEN">'.$comprobante->venta_total.'</cbc:Amount>
                     ...
                     <cac:PaymentTerms>
                        ...
                        <cbc:Amount currencyID="PEN">'.$comprobante->venta_total.'</cbc:Amount>
                        <cbc:PaymentDueDate>'.$dia_cuota.'</cbc:PaymentDueDate>
                    </cac:PaymentTerms>';*/
        }
        ...
        foreach($detalle as $v){
            $cantidadservicio = $v->venta_detalle_cantidad;
            $precioUnitario = $v->venta_detalle_precio_unitario;

            $xml.='<cac:InvoiceLine>
               <cbc:ID>'.$item.'</cbc:ID>
               <cbc:InvoicedQuantity unitCode="'.$v->medida_codigo_unidad.'">'.$cantidadservicio.'</cbc:InvoicedQuantity>
            ...
            if($v->venta_detalle_descuento > 0){
                $precio_producto = $v->venta_detalle_importe_total / $cantidadservicio;
            }else{
                $precio_producto = $precioUnitario;
            }
            if($v->codigo == "21"){
                $xml.= '<cac:AlternativeConditionPrice>
                     <cbc:PriceAmount currencyID="'.$comprobante->abrstandar.'">'.$precio_producto.'</cbc:PriceAmount>
            ...
            $xml.= '<cac:TaxTotal>
                      <cbc:TaxAmount currencyID="'.$comprobante->abrstandar.'">'.round($v->venta_detalle_total_igv, 2).'</cbc:TaxAmount>
                      <cac:TaxSubtotal>
...
//            if($v->venta_detalle_nombre_producto == "BOLSA"){
//                $total_impuesto_bolsa = number_format($cantidadservicio * $icbper, 2);
//                $xml.= '<cac:TaxSubtotal>
//                            <cbc:TaxAmount currencyID="'.$comprobante->abrstandar.'">'.$total_impuesto_bolsa.'</cbc:TaxAmount>
//                            <cbc:BaseUnitMeasure unitCode="'.$v->medida_codigo_unidad.'">'.$cantidadservicio.'</cbc:BaseUnitMeasure>
...
//            }
            ...
            $xml.= '<cac:Item>
                      <cbc:Description><![CDATA['.$v->venta_detalle_descripcion.']]></cbc:Description>
                      <cac:SellersItemIdentification>
                         <cbc:ID>'.$v->id_servicio.'</cbc:ID> 
                      </cac:SellersItemIdentification>
                   </cac:Item>';
            ...
            if($v->codigo == "21"){
                ...
                $xml.= '<cac:Price>
                  <cbc:PriceAmount currencyID="'.$comprobante->abrstandar.'">'.$precioUnitario.'</cbc:PriceAmount>
               </cac:Price>
            </cac:InvoiceLine>';
            }
    ...
    function CrearXMLNotaDebito($nombrexml, $emisor, $cliente, $comprobante, $detalle, $descripcion_nota)
    {
      ...
      <DebitNote xmlns="urn:oasis:names:specification:ubl:schema:xsd:DebitNote-2" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2" xmlns:ds="http://www.w3.org/2000/09/xmldsig#" xmlns:ext="urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2">
        ...
        foreach($detalle as $v){
            $precionunitario = $v->venta_detalle_precio_unitario;
            ...
            if($v->codigo == "21"){
                $xml.= '<cac:AlternativeConditionPrice>
                     <cbc:PriceAmount currencyID="'.$comprobante->abrstandar.'">'.$precionunitario.'</cbc:PriceAmount>
                     <cbc:PriceTypeCode>02</cbc:PriceTypeCode>
                  </cac:AlternativeConditionPrice>';
            }else {
                $xml .= '<cac:AlternativeConditionPrice>
