...
class Menu{
    ...
    public function guardar_menu($model){
        try{
            if(isset($model->id_menu)){
                ...
                $sql = 'insert into menus (menu_nombre,id_grupo, menu_controlador, menu_icono, menu_orden, menu_mostrar, menu_estado) values (?,?,?,?,?,?,?)';
                $stm = $this->pdo->prepare($sql);
                $stm->execute([
                    $model->menu_nombre,
                    $model->id_grupo,
    ...
    public function validar_grupo($nombre){
        try{
            $sql = 'select * from grupos where grupo_nombre = ?';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$nombre]);
            return $stm->fetch();
        } catch (Throwable $e){
    ...
    public function validar_nombre_edicion($nombre,$id){
        try{
            $sql = 'select * from grupos where grupo_nombre = ? and id_grupo <> ?';
            $stm = $this->pdo->prepare($sql);
    ...
    public function doble_permiso($id_grupo,$id_rol){
        try{
            $sql = 'select * from roles_grupos where id_grupo = ? and id_rol = ?';
            $stm = $this->pdo->prepare($sql);
            $stm->execute([$id_grupo,$id_rol]);
            return $stm->fetch();
        } catch (Throwable $e){
            $this->log->insertar($e->getMessage(), get_class($this).'|'.__FUNCTION__);
            return [];
